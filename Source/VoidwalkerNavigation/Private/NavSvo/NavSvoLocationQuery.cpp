...
DECLARE_CYCLE_STAT(TEXT("FindClosestNode"), STAT_FindClosestNode, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("FindReachableNodes"), STAT_FindReachableNodes, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("FindClosestReachableNode"), STAT_FindClosestReachableNode, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("FindRandomReachableNode"), STAT_FindRandomReachableNode, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("SearchReachableNode"), STAT_SearchReachableNodes, STATGROUP_VoidwalkerNavigation);
			...
			if (RelevantNode.Node->IsLeafNode())
			{
				...
				for (FSvoVoxelIterator VoxelIter; VoxelIter; ++VoxelIter)
				{
					...
					if (!bVoxelIsBlocked)
					{
						...
						if (FVoidwalkerNavigationUtils::AABBIntersectsAABB(QueryBounds, VoxelBounds))
						{
			...
			else
			{
				for (uint8 ChildIdx = 0; ChildIdx < 8; ++ChildIdx)
				{
					...
					if (ChildNode->GetNodeState() != ENodeState::Blocked)
					{
						...
						if (FVoidwalkerNavigationUtils::AABBIntersectsAABB(QueryBounds, NodeBounds))
						{
							FVector ClosestPointOnNode = RelevantNode.Bounds.GetClosestPointTo(Origin);
...
FSvoNodeLink FNavSvoNodeQuery::FindClosestReachableNode(const FVector& Origin, float DistanceLimit, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults)
{
...
FSvoNodeLink FNavSvoNodeQuery::FindRandomReachableNode(const FVector& Origin, float DistanceLimit, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults)
{
...
bool FNavSvoNodeQuery::SearchReachableNodes(const FVector& Origin, float DistanceLimit, TFunction<bool(NavNodeRef)> InNodeVisitedCallback, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults)
{
