...
bool FNavSvoTileGenerator::AddTile(const FIntVector& TileCoord)
{
	...
	for (const FBox& InclusionBounds : Parent->GetInclusionBounds())
	{
		...
		if (FVoidwalkerNavigationUtils::AABBIntersectsAABB(InclusionBounds, PaddedBounds))
		{
			const FBox ClippedGenerationBounds = FVoidwalkerNavigationUtils::CalculateAABBOverlap(InclusionBounds.ExpandBy(Config.BoundsPadding), PaddedBounds);
		...
		if (FVoidwalkerNavigationUtils::AABBIntersectsAABB(InclusionBounds, TileBounds))
		{
