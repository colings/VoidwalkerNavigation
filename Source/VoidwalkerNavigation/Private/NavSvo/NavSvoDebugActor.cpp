...
void ANavSvoDebugActor::RefreshNavData()
{
	if (UNavigationSystemV1* NavSys = Cast<UNavigationSystemV1>(GetWorld()->GetNavigationSystem()))
	{
		...
		NavAgentProperties.SetPreferredNavData(AVoidwalkerNavData::StaticClass());

		// Find the navigation data that best fits the specified agent properties.
		NavData = Cast<AVoidwalkerNavData>(NavSys->GetNavDataForProps(NavAgentProperties));
	}
	else
	{
		NavData = nullptr;
...
void ANavSvoDebugActor::RebuildPath()
{
	...
	FVoidwalkerNavQueryFilter* NavFilterImpl = static_cast<FVoidwalkerNavQueryFilter*>(NavQueryFilter->GetImplementation());
	...
	if (!bStringPullPath)
	{
		NavQuery.NavDataFlags |= (uint32)EVoidwalkerNavPathFlags::SkipStringPulling;
	}

	if (!bSmoothPath)
	{
		NavQuery.NavDataFlags |= (uint32)EVoidwalkerNavPathFlags::SkipSmoothing;
	}
	...
	if (NavPath.IsValid())
	{
		const FVoidwalkerNavPath* NavPath3D = static_cast<FVoidwalkerNavPath*>(PathResult.Path.Get());

		bPathNodeLimitReached = bPathExists ? NavPath3D->DidSearchReachedLimit() : false;

		const FVoidwalkerNavPathQueryResults& GenerationInfo = NavPath3D->GetGenerationInfo();
...
void ANavSvoDebugActor::DrawPath(UWorld* World) const
{
	...
	const FVoidwalkerNavPath* NavPath3D = StaticCast<FVoidwalkerNavPath*>(NavPath.Get());
