...
DECLARE_CYCLE_STAT(TEXT("SearchNodes"), STAT_SearchNodes, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("OpenNeighbor"), STAT_OpenNeighbor, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("OpenNeighborChildren"), STAT_OpenNeighborChildren, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("OpenVoxelsOnNeighborNode"), STAT_OpenVoxelsOnNeighborNode, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("OpenNeighbors"), STAT_OpenNeighbors, STATGROUP_VoidwalkerNavigation);
...
FNavSvoNode* FNavSvoQuery::TryAddSearchNode(FSvoNodeLink NodeLink)
{
	...
	if (NewNode == nullptr)
	{
		Results->Status |= (uint8)EVoidwalkerNavQueryFlags::OutOfNodes;
	}
...
bool FNavSvoQuery::SearchNodes(FSvoNodeLink InStartNodeLink, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults)
{
	...
	{
		Results->Status = (uint8)(EVoidwalkerNavQueryFlags::Failure | EVoidwalkerNavQueryFlags::OutOfMemory);
		return false;
	}
	...
	if (!Octree.IsValid())
	{
		Results->Status = (uint8)(EVoidwalkerNavQueryFlags::Failure | EVoidwalkerNavQueryFlags::InvalidParam);
		return false;
	}
	...
	{
		Results->Status |= (uint8)EVoidwalkerNavQueryFlags::Failure;
		return false;
	}
	...
	Results->Status |= (uint8)EVoidwalkerNavQueryFlags::Success;
	return true;
}
...
float FNavSvoQuery::GetHeuristic(FSvoNodeLink FromLink) const
{
	...
	const float Cost = FVoidwalkerNavigationUtils::GetManhattanDistance(FromCoord, GoalCoord);
