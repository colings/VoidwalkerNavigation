...
// Profiling stats
DECLARE_CYCLE_STAT(TEXT("ProcessTileTasks (FNavSvoGenerator)"), STAT_NavSvoGenerator_ProcessTileTasks, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("AddGeneratedTiles (FNavSvoGenerator)"), STAT_NavSvoGenerator_AddGeneratedTiles, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("MarkDirtyTiles (FNavSvoGenerator)"), STAT_NavSvoGenerator_MarkDirtyTiles, STATGROUP_VoidwalkerNavigation);

// Memory stats
DECLARE_MEMORY_STAT(TEXT("Pending Tiles (FNavSvoGenerator)"), STAT_NavSvoGenerator_PendingTiles, STATGROUP_VoidwalkerNavigation);
DECLARE_MEMORY_STAT(TEXT("Building Tiles (FNavSvoGenerator)"), STAT_NavSvoGenerator_BuildingTiles, STATGROUP_VoidwalkerNavigation);
...
FNavSvoGenerator::FNavSvoGenerator(AVoidwalkerNavData* InNavDataActor)
...
void FNavSvoGenerator::TickAsyncBuild(float DeltaSeconds)
{
	...
	const int32 MaxTileGeneratorTasks = AVoidwalkerNavData::IsGenerationBoostMode() ?
...
int32 FNavSvoGenerator::TickBuildTasks(const int32 MaxTasksToSubmit)
{
	...
	const double MaxTickMS = AVoidwalkerNavData::IsGenerationBoostMode() ?
...
void FNavSvoGenerator::UpdateNavigationBounds()
{
	...
	if (NavigationSystem->ShouldGenerateNavigationEverywhere() == false)
	{
		if (NavDataActor)
		{
			...
			{
				if (!FVoidwalkerNavigationUtils::AABBsContainAABB(InclusionBounds, Bounds))
				{
...
void FNavSvoGenerator::MarkDirtyTiles(const TArray<FNavigationDirtyArea>& DirtyAreas)
{
	...
	for (const FNavigationDirtyArea& DirtyArea : DirtyAreas)
	{
		...
		{
			if (!FVoidwalkerNavigationUtils::AABBIntersectsAABBs(AdjustedAreaBounds, InclusionBounds))
			{
				...
				if (!FVoidwalkerNavigationUtils::AABBIntersectsAABB(OctreeBounds, AdjustedAreaBounds))
				{
					continue;
				}
				...
				AdjustedAreaBounds = FVoidwalkerNavigationUtils::CalculateAABBOverlap(OctreeBounds, AdjustedAreaBounds);
		...
		else
		{
			...
			if (!FVoidwalkerNavigationUtils::AABBIntersectsAABB(TotalNavBounds, AdjustedAreaBounds))
			{
				continue;
			}
			...
			AdjustedAreaBounds = FVoidwalkerNavigationUtils::CalculateAABBOverlap(TotalNavBounds, AdjustedAreaBounds);
			...
			bNeedsTileIntersectionTest = !FVoidwalkerNavigationUtils::AABBsContainAABB(InclusionBounds, AdjustedAreaBounds);
		}
		...
		for (FCoordIterator CoordIter(MinAreaCoord, MaxAreaCoord); CoordIter; ++CoordIter)
		{
			...
			if (DirtyTile == nullptr)
			{
				...
				if (bNeedsTileIntersectionTest)
				{
					...
					if (!FVoidwalkerNavigationUtils::AABBIntersectsAABBs(TileBounds, InclusionBounds))
					{
						continue;
					}
				}
...
void FNavSvoGenerator::SortPendingTiles()
{
	...
	{
		FVoidwalkerNavigationUtils::GetPlayerLocations(World, SeedLocations);
	}
