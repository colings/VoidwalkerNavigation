...
FNavSvoSceneProxy::FNavSvoSceneProxy(const UPrimitiveComponent* InComponent, const AVoidwalkerNavData* NavData)
	: FPrimitiveSceneProxy(InComponent)
	, VertexFactory(GetScene().GetFeatureLevel(), "FNavSvoSceneProxy")
{
	NavDebugMaterial = LoadObject<UMaterial>(nullptr, TEXT("/VoidwalkerNavigation/VertexColorWireframeMaterial"));
...
void FNavSvoSceneProxy::GatherData(const AVoidwalkerNavData* NavData, TArray<FTileBuildData>& TilesOut) const
{
	if (NavData)
	{
...
bool FNavSvoSceneProxy::ShouldDraw(const AVoidwalkerNavData* NavData, bool bIsBlocked) const
{
	return
