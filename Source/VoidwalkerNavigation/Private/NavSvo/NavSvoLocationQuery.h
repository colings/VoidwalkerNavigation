...
public:
	...
	FSvoNodeLink FindClosestReachableNode(const FVector& Origin, float DistanceLimit, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults);

	// Finds a reachable location from the supplied origin.
	FSvoNodeLink FindRandomReachableNode(const FVector& Origin, float DistanceLimit, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults);

	// Collects all reachable nodes from the supplied origin.
	bool SearchReachableNodes(const FVector& Origin, float DistanceLimit, TFunction<bool(NavNodeRef)> InNodeVisitedCallback, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavQueryResults& InOutResults);
