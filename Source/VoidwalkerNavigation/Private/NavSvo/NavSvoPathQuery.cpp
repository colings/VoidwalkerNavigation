...
DECLARE_CYCLE_STAT(TEXT("FindPath (Query)"), STAT_FindPath_Query, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("TestPath (Query)"), STAT_TestPath_Query, STATGROUP_VoidwalkerNavigation);

FNavSvoPathQuery::FNavSvoPathQuery(const FSparseVoxelOctree& InOctree, int32 MaxSearchNodes)
	: Super(InOctree, MaxSearchNodes)
{}

bool FNavSvoPathQuery::FindPath(FSvoNodeLink InStartNodeLink, FSvoNodeLink InGoalNodeLink, float InCostLimit, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavPathQueryResults& InOutResults)
{
	...
	{
		InOutResults.Status = (uint8)(EVoidwalkerNavQueryFlags::Failure | EVoidwalkerNavQueryFlags::InvalidParam);
		return false;
	}
	...
	if (InStartNodeLink == InGoalNodeLink)
	{
		...
		if (BestSearchNode != nullptr)
		{
			...
			InOutResults.Status |= (uint8)EVoidwalkerNavQueryFlags::Success;
			return true;
		}

		InOutResults.Status |= (uint8)EVoidwalkerNavQueryFlags::Failure;
		return false;
	}
	...
	if (QueryResult)
	{
		...
		{
			InOutResults.Status |= (uint16)EVoidwalkerNavPathQueryFlags::PartialPath;
		}
		...
		do
		{
			...
			{
				InOutResults.Status |= (uint16)EVoidwalkerNavPathQueryFlags::CyclicalPath;
				break;
			}
...
bool FNavSvoPathQuery::TestPath(FSvoNodeLink InStartNodeLink, FSvoNodeLink InGoalNodeLink, float InCostLimit, const FVoidwalkerNavQueryFilter& InFilter, FVoidwalkerNavPathQueryResults& InOutResults)
{
