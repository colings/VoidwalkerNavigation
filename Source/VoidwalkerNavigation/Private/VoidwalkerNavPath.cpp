...
const FNavPathType FVoidwalkerNavPath::Type;

FVoidwalkerNavPath::FVoidwalkerNavPath()
	: bStringPull(true)
	, bSmooth(true)
{
	PathType = FVoidwalkerNavPath::Type;
}

void FVoidwalkerNavPath::ApplyFlags(uint32 NavDataFlags)
{
	if (NavDataFlags & (uint32)EVoidwalkerNavPathFlags::SkipStringPulling)
	{
		bStringPull = false;
	}

	if (NavDataFlags & (uint32)EVoidwalkerNavPathFlags::SkipSmoothing)
	{
		bSmooth = false;
	}
}

void FVoidwalkerNavPath::ResetForRepath()
{
...
void FVoidwalkerNavPath::DebugDraw(const ANavigationData* NavData, const FColor PathColor, class UCanvas* Canvas, const bool bPersistent, const float LifeTime, const uint32 NextPathPointIndex) const
{
