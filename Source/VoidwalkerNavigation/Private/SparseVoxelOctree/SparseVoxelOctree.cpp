...
// Profiling stats
DECLARE_CYCLE_STAT(TEXT("Generate (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_Generate, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("PopulateNode (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_PopulateNode, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("EnsureChildrenForNode (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_EnsureChildrenForNode, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("LinkNeighbors (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_LinkNeighbors, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("EnsureNodeExists (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_EnsureNodeExists, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("EnsureTileActiveAtCoord (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_EnsureTileActiveAtCoord, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("ReleaseTile (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_ReleaseTile, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("FindNodeLinkForLocation (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_FindNodeLinkForLocation, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("Raycast (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_Raycast, STATGROUP_VoidwalkerNavigation);
DECLARE_CYCLE_STAT(TEXT("GetActiveTileCoords (FSparseVoxelOctree)"), STAT_FSparseVoxelOctree_GetActiveTileCoords, STATGROUP_VoidwalkerNavigation);
...
bool FSparseVoxelOctree::Raycast(const FVector& RayStart, const FVector& RayEnd, VoidwalkerNavigation::FRaycastResult& Result) const
{
	...
	GetTilesInBounds(RayBounds, [&](const FSvoTile& Tile)
	{
		...
		if (FVoidwalkerNavigationUtils::RayAABBIntersect(RayStart, RayDir, TileBounds, TileMinT, TileMaxT))
		{
...
bool FSparseVoxelOctree::RaycastTile(const FTileRaycastInfo& Info, VoidwalkerNavigation::FRaycastResult& Result) const
{
	...
	while (CurNodeLink.IsValid())
	{
		...
		auto AdvanceRay = [this, &CurNodeLink, &Info, &CurrentRayT, &CurrentRayLocation]() -> EAdvanceRay
		{
			...
			const bool bIntersects = FVoidwalkerNavigationUtils::RayAABBIntersect(Info.RayStart, Info.RayDir, NodeBounds, NodeMinT, NodeMaxT);
			//ensure(bIntersects);
