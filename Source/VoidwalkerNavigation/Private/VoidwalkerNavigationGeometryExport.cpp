...
#include "VoidwalkerNavigationGeometryExport.h"

#include "PhysicsEngine/BodySetup.h"

namespace VoidwalkerNavigationGeometryExport
{
	void ExportChaosHeightFieldSlice(const FNavHeightfieldSamples& PrefetchedHeightfieldSamples, const int32 NumRows, const int32 NumCols, const FTransform& LocalToWorld
		...
		, FBox& UnrealBounds)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_VoidwalkerNavigationGeometryExport_ExportHeightFieldSlice);
		...
		{
			QUICK_SCOPE_CYCLE_COUNTER(STAT_VoidwalkerNavigationGeometryExport_AllocatingMemory);
...
//////////////////////////////////////////////////////////////////////////
FVoidwalkerNavigationGeometryExport::FVoidwalkerNavigationGeometryExport(FNavigationRelevantData& InData)
	: Data(InData)
{
	Data.Bounds = FBox(ForceInit);
}

void FVoidwalkerNavigationGeometryExport::StoreCollisionCache()
{
...
void FVoidwalkerNavigationGeometryExport::ExportChaosTriMesh(const Chaos::FTriangleMeshImplicitObject* const TriMesh, const FTransform& LocalToWorld)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}

void FVoidwalkerNavigationGeometryExport::ExportChaosConvexMesh(const FKConvexElem* const Convex, const FTransform& LocalToWorld)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}

void FVoidwalkerNavigationGeometryExport::ExportChaosHeightField(const Chaos::FHeightField* const Heightfield, const FTransform& LocalToWorld)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}

void FVoidwalkerNavigationGeometryExport::ExportChaosHeightFieldSlice(const FNavHeightfieldSamples& PrefetchedHeightfieldSamples, const int32 NumRows, const int32 NumCols, const FTransform& LocalToWorld, const FBox& SliceBox)
{
	VoidwalkerNavigationGeometryExport::ExportChaosHeightFieldSlice(PrefetchedHeightfieldSamples, NumRows, NumCols, LocalToWorld, VertexBuffer, IndexBuffer, SliceBox, Data.Bounds);
}

void FVoidwalkerNavigationGeometryExport::ExportRigidBodySetup(UBodySetup& BodySetup, const FTransform& LocalToWorld)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}

void FVoidwalkerNavigationGeometryExport::ExportCustomMesh(const FVector* InVertices, int32 NumVerts, const int32* InIndices, int32 NumIndices, const FTransform& LocalToWorld)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}

void FVoidwalkerNavigationGeometryExport::AddNavModifiers(const FCompositeNavModifier& Modifiers)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}

void FVoidwalkerNavigationGeometryExport::SetNavDataPerInstanceTransformDelegate(const FNavDataPerInstanceTransformDelegate& InDelegate)
{
	// This class is only used for exporting heightfield slices, nothing else should be called
	checkNoEntry();
}